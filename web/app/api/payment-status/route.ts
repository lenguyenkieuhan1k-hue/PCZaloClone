...
export async function GET(req: NextRequest) {
  ...
  const { data: rows, error } = await userClient
    .from('payments')
    .select('id, status, memo, created_at, paid_at, sepay_txn_id')
    .eq('user_id', userId)
  ...
  if (matched.status === 'paid') return NextResponse.json({ ok: true, status: 'paid', matched: true })
  if (matched.status === 'failed') return NextResponse.json({ ok: true, status: 'failed', matched: true })
  return NextResponse.json({ ok: true, status: 'pending', matched: true })
}
